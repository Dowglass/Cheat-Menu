...
script_version_number(2020102602) -- YYYYMMDDNN
...
function(self) -- render frame
    ...
    if fmenu.tmenu.update_status == fconst.UPDATE_STATUS.NEW_UPDATE then
        imgui.Button("Uma versão atualizada está disponivel!",imgui.ImVec2(fcommon.GetSize(1)))
    ...
    if tcheatmenu.current_menu == 0 then
        ...
        if imgui.BeginChild("Bem-vindo") then
            ...
            imgui.TextWrapped("Bem-vindo ao " .. tcheatmenu.title .. " by Grinch_")
            imgui.Dummy(imgui.ImVec2(0,10))
            ...
            imgui.Spacing()
            fcommon.HotKey("Tecla para abrir/fechar o Cheat Menu",fmenu.tmenu.hot_keys.menu_open)
...
function()
    ...
    if isCharInAnyCar(PLAYER_PED) then
        ...
        if  fmenu.tmenu.overlay.speed[0] then
            imgui.Text(string.format("Velocidade: %d %d/%d",math.floor(getCarSpeed(hveh)),getCarCurrentGear(hveh)
            ,getCarNumberOfGears(hveh)))
        end

        if fmenu.tmenu.overlay.health[0] then
            imgui.Text(string.format("Saúde: %.0f%%",getCarHealth(hveh)/10))
        end
    end

    if fmenu.tmenu.overlay.location[0] then
        imgui.Text("Local: " .. fcommon.GetLocationInfo(getCharCoordinates(PLAYER_PED)))
    end
...
function()
    ...
    if imgui.InputTextWithHint("##TEXTFIELD","Digite o comando",fmenu.tmenu.command.input_field,ffi.sizeof(fmenu.tmenu.command.input_field),imgui.InputTextFlags.EnterReturnsTrue 
        ...
        if fconfig.tconfig.reset == false then
            if fmenu.tmenu.crash_text == "" then
                ...
                if fmenu.tmenu.auto_reload[0] and not fgame.tgame.script_manager.skip_auto_reload then
                    script:reload()
                    fmenu.tmenu.crash_text =  fmenu.tmenu.crash_text .. " Script recarregado."
                end
            end
        end
