...
function module.AnimationMain()
    ...
    if fcommon.BeginTabBar('Animações') then
        ...
        if fcommon.BeginTabItem('Misc') then
            if fcommon.DropDownListNumber("Luta",module.tanimation.fighting.names,module.tanimation.fighting.selected) then
                giveMeleeAttackToChar(PLAYER_PED,module.tanimation.fighting.selected[0]+3,6)
                fcommon.CheatActivated()
            end
            if fcommon.DropDownListNumber("Caminhada",module.tanimation.walking.names,module.tanimation.walking.selected) then
            ...
            imgui.InputTextWithHint("Animação##input","anim",module.tanimation.name,ffi.sizeof(module.tanimation.name))
            imgui.Spacing()
