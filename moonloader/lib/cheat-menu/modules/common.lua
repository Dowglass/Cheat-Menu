...
function module.LoadJson(filename)
    ...
    if doesFileExist(full_path) then
        ...
        if status and table then
            ...
            print("Falha ao carregar o arquivo json, " .. filename)
        end
    end
    return {}
end
...
function module.MoveFiles(main_dir,dest_dir)
    for f in lfs.dir(main_dir) do
        ...
        if doesFileExist(main_file) then
            ...
            if doesFileExist(dest_file) then
                os.remove(main_file)
                print("Impossivel deletar o arquivo" .. dest_file)
            else
       ...
       imgui.InvisibleButton("c##".. label,imgui.CalcTextSize("c"))
        ...
        if imgui.IsItemClicked(0) then
            tcheatmenu.window.panel_func = function()
                imgui.TextWrapped(string.format("%s",label))
                imgui.Separator()
                if imgui.Button("Fechar",imgui.ImVec2(module.GetSize(1))) then
...
function module.CRGBAColorPicker(label,base_addr,default_col,save)
    ...
    fcommon.DropDownMenu(label,function()
        if imgui.ColorPicker4("Cor##" ..label,CRGBA) then
        ...
        imgui.Spacing()
        if imgui.Button("Restaurar padrão",imgui.ImVec2(module.GetSize(1))) then
...
function module.DrawEntries(identifier,draw_type,func_on_left_click,func_on_right_click,func_get_name,data_table,const_image_height,const_image_width)
    ...
    fcommon.DropDownList("##Lista",tcheatmenu.temp_data.draw_entries_func[identifier].entry_table,tcheatmenu.temp_data.draw_entries_func[identifier].selected,
    function(key,val) 
    ...
    tcheatmenu.temp_data.draw_entries_func[identifier].filter:Draw("##Filtrar")
    ...
    if imgui.BeginChild("##Desenhar") then 
...
function module.UpdateStat(arg)
    ...
    fcommon.DropDownMenu(arg.name,function()
        ...
        imgui.Spacing()
        if imgui.Button("Mínimo##".. arg.name,imgui.ImVec2(module.GetSize(3))) then
            setFloatStat(arg.stat,arg.min)
            fconfig.Set(fconfig.tconfig.stat_data,tostring(arg.stat),arg.min)
        end
        imgui.SameLine()
        if imgui.Button("Padrão##".. arg.name,imgui.ImVec2(module.GetSize(3))) then
            setFloatStat(arg.stat,arg.default)
            fconfig.Set(fconfig.tconfig.stat_data,tostring(arg.stat),arg.default)
        end
        imgui.SameLine()
        if imgui.Button("Máximo##".. arg.name,imgui.ImVec2(module.GetSize(3))) then
...
function module.UpdateAddress(arg)
    ...
    fcommon.DropDownMenu(arg.name,function()
        ...
        if buttons > 0 then
            imgui.Spacing()
            if imgui.Button("Mínimo##".. arg.name,imgui.ImVec2(module.GetSize(buttons))) then
            ...
            if arg.default ~= nil then
                imgui.SameLine()
                if imgui.Button("Padrão##".. arg.name,imgui.ImVec2(module.GetSize(buttons))) then
            ...
            if arg.max ~= nil then
                imgui.SameLine()
                if imgui.Button("Máximo##".. arg.name,imgui.ImVec2(module.GetSize(buttons))) then
...
function module.CheatDeactivated()
    printHelpString("Cheat ~r~Desativado")
end
...
function module.BeginTabBar(label)
    ...
    if #_tabs[label].bar_data > 0 then
        ...
        imgui.InvisibleButton("?##Iformação",imgui.CalcTextSize("?"))
        ...
        if imgui.IsItemHovered() then
            imgui.BeginTooltip()
            imgui.SetTooltip("Se a largura da sua janela for pequena, você poderá rolar com scrool do mouse")
            imgui.EndTooltip()
        end
        imgui.EndChild()
        imgui.Spacing()
