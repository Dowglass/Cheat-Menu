    ...
    if func ~= nil then
        ...
        if imgui.IsItemClicked(0) then
            tcheatmenu.window.panel_func = function()
                imgui.TextWrapped(string.format("%s configurações",label))
                imgui.Separator()
...
function module.Tabs(label,names,func)
    ...
    if imgui.BeginChild(label,imgui.ImVec2(tcheatmenu.window.size.X-imgui.GetStyle().WindowPadding.x*2,y*0.9)) then
        ...
        fcommon.InformationTooltip("Se a largura da sua janela for pequena, você poderá rolar com scrool do mouse.")
...
function module.MoveFiles(main_dir,dest_dir)
    for f in lfs.dir(main_dir) do
        ...
        if doesFileExist(main_file) then
            ...
            if doesFileExist(dest_file) then
                os.remove(main_file)
                print("[Aviso]Nao foi possivel excluir o arquivo " .. dest_file)
            else
